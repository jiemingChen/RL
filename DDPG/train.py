...
def continuos_cartpole_train(n_episodes=2000, max_t=700):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if np.mean(scores_deque) > 200:
            torch.save(agent.alg.model.actor_model.state_dict(), 'cart_pole_actor.pth')
            torch.save(agent.alg.model.critic_model.state_dict(), 'cart_pole_critic.pth')
            break
    return scores


#continuos_cartpole_train()
...
def evaluate( render=True):
    ...
    alg =  DDPG(model,target_model, gamma=0.99, tau=1e-3, actor_lr=1e-4, critic_lr=3e-4)
    agent = Agent(alg, BUFFER_SIZE, BATCH_SIZE, seed=10)
    agent.alg.model.actor_model.load_state_dict(torch.load("cart_pole_actor.pth"))
    agent.alg.model.critic_model.load_state_dict(torch.load("cart_pole_critic.pth"))
